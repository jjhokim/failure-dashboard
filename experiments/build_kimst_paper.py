...
SEC1_TITLE = "1. 서론"
SEC1 = (
    "무기체계의 가용도 향상과 총수명주기비용 관리를 위해 MRO 관점의 정비·고장 데이터 "
    "수집과 품질 관리가 선결 과제로 제시된 바 있다[1]. 그러나 고장분류 코드는 경향을 "
    "식별할 만큼의 상세도를 갖지 못하는 경우가 많고, 실제 정보는 정비사가 작성한 "
    "자유서술에 담겨 있다[2]. "
    "본 연구는 수집 이후 단계, 즉 축적된 자유서술 정비기록에서 동일 원인에 기인할 수 "
    "있는 고장군(공통원인 후보)을 선별하고 조치 우선순위를 매기는 스크리닝 "
    "파이프라인을 제안한다. "
    "다만 합성데이터로 군집화를 평가할 때는 순환검증 위험이 있다. 정답 라벨이 관측 "
    "특징으로 흘러들어가면 측정된 성능은 모델의 능력이 아니라 신호 주입 강도를 재게 "
    "된다[3]. 이를 설계 단계에서 차단한 점을 주요 기여로 한다."
)
...
SEC21 = (
    ...
    "문장 임베딩의 코사인 거리, 구조 채널은 군수관리번호(LCN) 계층트리 경로거리를 "
    "최대깊이로 정규화한 값, 시간 채널은 1-exp(-|Δt|/τ)이다(τ=30일). 각 채널을 "
    "min-max 정규화 후 가중합하여 사전계산 거리 기반 HDBSCAN으로 군집화한다. "
    "가중치를 0으로 두면 채널이 꺼지므로 동일 구현으로 절제(ablation) 실험이 가능하다. "
    "군집 점수는 규모·추세·치명도를 표준화해 가중합하며, 탐지기는 주입된 치명도 "
    "파라미터를 알 수 없으므로 관측치 기반 대체값만 사용한다."
)
...
SEC23 = (
    "합격기준을 실행 전에 사전등록하고 모든 수치는 8개 시드의 평균±표준편차로 "
    "보고했다(600건, 10개 군집). Table 1과 같이 무주입 및 라벨 순열 대조군에서 ARI가 "
    "0으로 붕괴해 순환성이 없음을 확인했고 주입 강도에 따라 ARI는 단조 증가했다. "
    "그러나 3채널 융합(0.736)이 시간 채널 단독(0.853)에 미치지 못해 융합 우위 기준은 "
    "충족하지 못했다. 가중치를 탐색하자 최적해가 시간 채널 단독으로 수렴했는데, 이는 "
    "생성기가 δ=1.0에서 군집 발생 시각을 하루 이내로 과도하게 밀집시켜 시간 채널이 "
    "거의 완전한 판별자가 되었기 때문이다. 밀집폭을 관측기간의 1%로 완화하면 융합 "
    "0.938, 시간 단독 0.786으로 역전되고 차이의 95% 신뢰구간이 [0.090, 0.236]으로 "
    "0을 포함하지 않았으며, 이때 재탐색한 가중치는 세 채널을 모두 유지했다."
)
...
SEC3 = (
    ...
    "구현하고 순환검증을 차단한 설계로 내적 타당도를 검증했다. 핵심 결과는 채널 융합의 "
    "이점이 무조건적이지 않고 공통원인 신호의 시간 분산 정도에 조건부라는 점이다. "
    "한계는 다음과 같다. 평가가 합성데이터 기반이므로 실운용 성능을 주장하지 않는다. "
    "시간 밀집폭은 실측 근거가 없는 설계값이며, 공통원인 고장의 시간 창은 문헌상 임무와 "
    "점검주기에 따라 선택되는 값이다[4]. 또한 주입 강도가 낮은 구간에서는 신호가 "
    "회수되지 않아 본 도구는 조기경보가 아닌 사후 스크리닝으로 위치시켜야 한다. "
    "향후 실제 야전 정비기록으로 시간 밀집도를 추정하는 연구가 필요하다."
)
...
def build() -> Path:
    ...
    # 작성 안내문 제거 — 내용을 비우기만 하면 빈 단락이 남아 지면을 차지하므로
    # XML 노드 자체를 삭제한다(2페이지 제약 준수에 필요).
    for p in paras[DROP_FROM:]:
        p._element.getparent().remove(p._element)

    # 연속된 빈 단락 정리 (2개 이상 연속이면 하나만 남김)
    직전_빈줄 = False
    for p in list(doc.paragraphs):
        빈줄 = not p.text.strip() and not p.runs
        if 빈줄 and 직전_빈줄:
            p._element.getparent().remove(p._element)
        else:
            직전_빈줄 = 빈줄
    ...
    if FIG1_IMAGE.exists():
        ...
        if 캡션단락 is not None:
            ...
            # 2칼럼 폭(약 8.5cm)에 맞춘다
            그림.add_run().add_picture(str(FIG1_IMAGE), width=Cm(7.4))
    else:
